...
# Round H 전용 — App 은 docker-compose 컨테이너, DB 는 원격 박스(DB_SSH). 기존 스크립트의
# "3대 모드"(DB_SSH 설정, App=systemd)도 "2대 모드"(DB_SSH 미설정, App+DB 같은 박스) 어느
# 쪽에도 안 맞는 조합이라 이 플래그로 분기한다. DB_HOST_REMOTE 는 docker-compose 오버라이드에
# 박아 넣을 DB 박스의 사설 IP.
APP_CONTAINER=${APP_CONTAINER:-0}
DB_HOST_REMOTE=${DB_HOST_REMOTE:-}
# Round I 전용 — Round H(APP_CONTAINER=1, 브리지 네트워크+docker-compose)의 네트워크 모드만
# host 로 바꾼 대조판. docker-compose 는 network_mode:host 와 상위 파일의 networks: 를 같은
# 서비스에 동시에 못 가지므로(병합 시 충돌), compose 를 안 쓰고 `docker run --network host`로
# 직접 띄운다 — 이미지는 `docker compose build shadowfit-backend`로 미리 빌드해둔 것을 재사용.
APP_IMAGE=${APP_IMAGE:-init-shadowfit-backend}
GHZ_TOKEN_FOR_APP=${GHZ_TOKEN_FOR_APP:-}
...
if [ "$APP_CONTAINER" = "1" ]; then
  [ -n "$DB_HOST_REMOTE" ] || die "APP_CONTAINER=1 인데 DB_HOST_REMOTE 가 없다"
  note "Round H 모드 — App 은 컨테이너, DB 는 원격 박스($DB_HOST_REMOTE, DB_SSH=$DB_SSH)"
  $TARGET_SSH "test -d $TARGET_REPO_DIR && docker ps >/dev/null 2>&1" \
    || die "대상 박스에 못 붙거나 $TARGET_REPO_DIR / docker 가 없다"
  [ -n "$DB_SSH" ] || die "APP_CONTAINER=1 은 DB_SSH(원격 DB 지표 수집용)도 같이 필요하다"
...
apply_pool() {  # $1 = pool 크기
  local pool=$1
  if [ "$APP_CONTAINER" = "1" ]; then
    # Round I — Round H와 동일 이미지·환경이지만 docker-compose(브리지 네트워크) 대신
    # `docker run --network host`로 직접 띄운다. compose는 network_mode:host와 상위
    # docker-compose.yml의 networks: 를 같은 서비스에 동시에 못 가져서(병합 충돌) 우회한다.
    $TARGET_SSH "docker rm -f shadowfit-backend >/dev/null 2>&1
      docker run -d --name shadowfit-backend --network host --restart unless-stopped \
        --memory=2g --cpus=4 \
        -e SPRING_PROFILES_ACTIVE=prod -e TZ=Asia/Seoul \
        -e JAVA_OPTS='-XX:MaxRAMPercentage=75.0' -e SPRINGDOC_ENABLED=true \
        -e DB_HOST=$DB_HOST_REMOTE -e DB_USERNAME=shadowfit -e DB_PASSWORD=1234 \
        -e INTERNAL_API_TOKEN=$GHZ_TOKEN_FOR_APP \
        -e AI_SERVER_HOST=shadowfit-ai -e AI_SERVER_GRPC_PORT=8585 -e AI_CHANNEL_POOL_SIZE=3 \
        -e JWT_SECRET=shadowfit-prod-secret-key-change-this -e OPENAI_API_KEY= \
        -e POSE_INJECT_DELAY_MS=0 -e SHADOWFIT_POSE_DEADLOCK_BACKOFF_JITTER=false \
        -e SHADOWFIT_POSE_DEADLOCK_BACKOFF_MS=0 -e SHADOWFIT_POSE_DEADLOCK_MAX_RETRIES=5 \
        -e SPRING_DATASOURCE_HIKARI_MAXIMUM_POOL_SIZE=$pool \
        $APP_IMAGE" \
      || { echo "🔴 pool=$pool docker run 실패(APP_CONTAINER/host)" >&2; return 1; }
  elif [ -n "$DB_SSH" ]; then
