...
apply_pool() {  # $1 = pool 크기
  local pool=$1
  if [ "$APP_CONTAINER" = "1" ]; then
    # Round H — App 만 컨테이너로 띄운다(mysql 서비스는 안 씀, DB_HOST 를 원격 IP 로 오버라이드).
    # §11(3대) 과 토폴로지는 같고 "App 배포 방식" 하나만 bare jar → 컨테이너로 바뀐 것이라,
    # depends_on: mysql(service_healthy) 을 만족시킬 로컬 mysql 이 없다 — up 대상에서 뺀다.
...
services:
  shadowfit-backend:
    ...
    depends_on: {}
YML
      docker compose -f docker-compose.yml -f docker-compose.pool.yml up -d --no-deps shadowfit-backend" \
      || { echo "🔴 pool=$pool 구성 명령 실패(APP_CONTAINER)" >&2; return 1; }
  elif [ -n "$DB_SSH" ]; then
