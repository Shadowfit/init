...
>
> 2026-09-12 진척: #1~#5 가 09-11 당일 머지(PR 720·724·725·728·732). **#7 은 늘어져서가 아니라 프론트에 실시간 수신 클라이언트가 0개라서 #8·#9 뒤로 뺐고, 1차 테스트 전까지 클라이언트가 안 붙으면 푸시만으로 닫는다**([`social-cheer-and-group-feed.md`](../decisions/social-cheer-and-group-feed.md) §4-2). 남은 순서: #6 → #8 → #9 → #10 → #11 → #12 → (#7).
