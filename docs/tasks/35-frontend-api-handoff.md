...
| 항목 | 현재 | 뭐가 필요한가 |
|---|---|---|
| 런지 | DB 에 종목 행은 있으나 `analysis_supported=FALSE`. AI 분석기가 `squat_analyzer.py` 하나뿐 | AI 쪽 런지 분석기 → 그 뒤 `PATCH /admin/exercises/{id}/analysis-support` 로 켬. **종목 선택 화면은 `GET /exercises`(2026-09-22 신설)로 목록을 받는다** — `analysisSupported=false` 면 «준비 중», `exercise.tsx` 의 `exerciseId=1` 고정·`exerciseTypeOf()` switch 는 응답의 `id`·`code` 로 대체. 설계: [`../decisions/lunge-and-set-backend.md`](../decisions/lunge-and-set-backend.md) |
