...
마지막 업데이트: **2026-09-10** (§3-3 REST 미러 = 프로토콜 A/B 추가 — 이전 §3-2 는 2026-09-06, 대규모 반영은 2026-08-08 §0 참조)

> ⚠️ **이 판은 브랜치 `explore/grpc-webclient-ab` 기준이다.** §3-3 이 적는 REST 미러·`AiAnalysisClient` 두 구현체는 **`origin/main` 에 아직 없다.** main 만 보는 사람에게는 §3-3 이 통째로 «미래»다.
