...
마지막 업데이트: **2026-09-10** (§4-1 프로토콜 A/B 추가 — 이전 2026-08-08, 그때 2.5개월치를 §4 로 채웠다)

> 🔴 **또 구멍이 있다 — 2026-08-09 ~ 09-07 은 아직 안 훑었다.** §4-1 은 그 뒤(09-08~09) 커밋만 담는다. 이 한 달은 부하테스트·측정 라운드가 대부분이라 결합 계약 변경이 없을 «가능성이 높지만», **확인하고 적은 게 아니라 안 훑은 것**이다. 이 문서가 2.5개월 밀렸던 것과 같은 종류의 빈칸이므로 그대로 적어둔다.
...
## 4-1. 프로토콜 A/B — REST 미러 도입 (2026-09-08 ~ 09-09) 🆕

> ⚠️ **아직 `origin/main` 에 없다.** 브랜치 `explore/grpc-webclient-ab` 에만 있고 PR 도 없다. 그런데도 여기 적는 이유는 이 문서가 «결합 계약이 언제 어떻게 바뀌었나»를 담기 때문이고, 이 4커밋이 **RPC 표면·인증 경계·실패 처리 세 곳을 동시에 건드리기** 때문이다(README 갱신 트리거 1·3·4 전부 해당).

### f538cd5b — docs(decisions): gRPC vs WebClient 실측 비교 설계 (2026-09-08)

**결합 결과**: 코드 무변경. 다만 이 문서 §6 의 *"결합 표면이 줄어드는 방향"* 관찰을 재검토하게 만든 발견이 여기 적혔다 — **«어느 AI 프로세스로 보낼까»를 푸는 라우팅 메커니즘이 이미 두 개**라는 것. REST 경로는 `ai-nginx` 의 `map` 디렉티브(`X-AI-Worker`), gRPC 경로는 Spring 이 손으로 짠 `ManagedChannel` 풀 + `session_id % 3`. **그리고 후자에서만 실제 버그가 났다**(채널 인덱스와 포트를 잘못 짝지어 세션 6개가 워커 0으로 몰림 — 코드 주석에 남아 있다).

### a446807d — refactor(exercise): `AiAnalysisClient` 인터페이스 분리 (2026-09-08) ⭐ 실패 처리 변경

**Spring 측** — `ExerciseAnalysisService` 가 직접 들고 있던 gRPC 채널·스텁·인증 헤더를 `GrpcAiAnalysisClient` 로 이관. 에러를 `AiCallOutcome`(`Success`/`ClientRejected`/`TransientFailure`) 으로 정규화해 **프로토콜 특유 예외 타입이 호출자에 안 흐르게** 했다.

**결합 결과**: 🔴 **관측 계약이 하나 좁아졌다.** `shadowfit.ai.stop.result` 의 `outcome` 태그에서 `grpc-error`(옛 `StatusRuntimeException`)와 `error` 의 구분이 사라지고 `error` 하나로 합쳐졌다. **proto 0줄·AI 0줄 변경인데 대시보드가 보는 값이 바뀐 것** — 이 문서 상단 «범위 주의» 가 경고하는 바로 그 모양(파일 위치가 아니라 계약으로 판정)이다.

### 04a0ccd2 — feat(exercise): `WebClientAiAnalysisClient` (2026-09-09) ⭐ RPC 표면 추가

**Spring 측** — 두 번째 구현체. `spring-boot-starter-webflux` 추가(**클라이언트로만** — 서버는 그대로 Tomcat/MVC). `ai.client-type=grpc|webclient` 로 스위치, 기본은 `grpc`.

**결합 결과**: **라우팅 경로가 갈렸다.** gRPC 는 `ai-nginx` 를 건너뛰고 8585-8587 직결, WebClient 는 `ai-nginx`(8000) 를 거쳐 `X-AI-Worker` 헤더 라우팅을 재사용한다 — 즉 이 경로엔 **Spring 쪽 수동 채널 풀이 없다.** 위 f538cd5b 가 발견한 «메커니즘 중복» 의 한쪽을 실제로 안 쓰는 구현이 처음 생긴 것.

### 0bb5df19 — feat(ai-server): Spring→AI REST 미러 (2026-09-09) ⭐ RPC 표면 + 인증 경계

**FastAPI 측** — `POST /api/v1/internal/analysis/{extract-reference,start,reattach,stop}` 4개. 기존 `ExerciseServicer` 를 in-process 호출하는 얇은 어댑터라 **판정 로직은 복제되지 않았다.**

**결합 결과**: **AI 의 HTTP 표면이 두 등급으로 갈렸다.** 이 4개 경로는 `AI_PUBLIC_TOKEN`(앱 번들 배포값)이 아니라 `INTERNAL_API_TOKEN` 을 요구한다 — 안 그러면 #134/#230 이 막은 구멍이 이 4개에서 재발한다. **proto 밖 계약(토큰 경계)이 바뀐 사례**로, 이 문서가 §6 에서 «proto 만 훑으면 놓친다» 고 적은 유형이다.

### 📌 이 4커밋의 결합면 요약

| 축 | 변화 |
|---|---|
| RPC 표면 | gRPC 7개 그대로 + **REST 미러 4개 병존** (요청 방향만, 콜백 3개는 미러 없음) |
| 인증 | `INTERNAL_API_TOKEN` 이 gRPC 메타데이터 **+ HTTP 헤더** 두 자리에서 쓰인다 |
| 라우팅 | 요청 경로가 **두 갈래** — 채널 풀 직결(gRPC) vs `ai-nginx` 경유(REST) |
| 실패 처리 | `AiCallOutcome` 정규화 — 프로토콜 예외가 호출자에 안 흐른다. 지표 태그 1건 병합 |
| 판정 기준 | **무변경** — 미러가 같은 서비서를 부른다 |

🔴 **이 표는 «준비» 의 요약이지 «결과» 가 아니다.** gRPC vs WebClient 측정은 아직 0건이고, 어느 쪽을 채택할지도 미결정이다([`../decisions/grpc-webclient-empirical-comparison.md`](../decisions/grpc-webclient-empirical-comparison.md) §9).
...
|---------|---------|--------------|
| REST 통신 (WebClient) | 660e294, 0d89668 | gRPC 도입 후 8ac8248에서 잔재 제거. 🔄 **2026-09-09 에 다시 등장** — 04a0ccd2 (§4-1, 이번엔 실측 A/B 용) |
...
| **AI HTTP 내부 표면** (`/api/v1/internal/analysis/*`) | 0bb5df19 (§4-1) | — · ⚠️ 브랜치에만 존재 |
| **`AiAnalysisClient` 추상화** (프로토콜 스위치) | a446807d · 04a0ccd2 (§4-1) | — · ⚠️ 브랜치에만 존재 |
...
- 🔄 **2026-09 추가: «표면이 줄어든다» 가 §4-1 에서 한 번 더 깨졌다 — 이번엔 «일부러 늘렸다».** REST 미러 4개는 gRPC 4개를 대체하지 않고 **병존**한다. §4 의 `ReattachAnalysis` 가 *"멱등 규칙이 정반대라 일부러 나눈 것"* 이었다면, 이건 *"어느 쪽이 나은지 재려고 한시적으로 둘 다 둔 것"* 이다. 🔴 **한시성이 지켜지는지는 문서가 아니라 실측 완료 여부로 판정된다** — 측정이 0건인 채로 시간이 지나면 이건 그냥 두 벌 유지 비용이다.
