...
| 운동 세션 완료 반영 | gRPC `ExerciseService.CompleteAnalysis` (AI → Spring) | totalReps·싱크통계를 서버가 `pose_data` 에서 집계해 COMPLETED 처리. 앱이 결과를 직접 보고하던 `PUT .../complete` 는 제거됨 (ET-H) |
...
| **회원 목록 조회** | `GET /admin/members` | `ROLE_ADMIN`. 필터 5(가입일 범위·검색어·페르소나·운동레벨·온보딩) · 정렬 3 · offset 페이징 |
| **세션 목록 조회** | `GET /admin/sessions` | `ROLE_ADMIN`. 필터 4(상태·운동종목·기간·회원검색어) · 정렬 3 · offset 페이징 |
| **대시보드 통계** | `GET /admin/stats/overview` | `ROLE_ADMIN`. 위젯 5종(오늘 세션 수·상태별 분포·평균 싱크로율·신규 가입자·활성 회원) |

> 📌 **아래 셋은 초판 요구사항에 없던 항목이다** (2026-08-07 편입). 관리자 화면 범위를 정하면서
> 새로 정의했고, 근거·측정은 [`decisions/admin-page-scope.md`](./decisions/admin-page-scope.md) 에 있다.
> 편입 이유는 진척률 계산의 분모를 실제 산출물과 맞추기 위해서다 — 구현·측정·문서가 다 끝난 기능이
> 요구사항 목록에 없으면 개수 기반 % 가 실제보다 낮게 나온다.
>
> 📌 **관리자 계정은 회원가입 시 `role` 을 지정해 만든다** (`MemberRequestDto.role`).
> 권한을 클라이언트가 정하는 형태이므로 운영 서비스라면 권한 상승 경로지만,
> **이 프로젝트는 데모 범위라 그대로 둔다** (2026-08-07 확인). 관리자 계정을 시연에서
> 바로 만들 수 있어야 하고, 승격 전용 경로를 따로 만드는 것은 범위 밖이다.
