...
관찰:
- **에러 ~1% 발생 (재현됨)** — ramp 표본 10,166 중 `Unavailable`(rpc error: read tcp ... use of closed network connection) **100건, 에러율 0.98%**. 1차·2차 ramp 모두 ~100건으로 재현 → 비결정적 noise 아님. 동시성 100 근처에서 커넥션이 리셋됨. **원인 추적 필요**(서버 gRPC max-connections/keepalive, OS 소켓 한도, 또는 부하 중 GC stall 로 인한 연결 끊김).
...
- **throughput 은 오히려 낮음 (RPS 12.9 → 48.4)** — 동시성을 1→100 으로 100배 올렸는데 처리량은 ~3.7배만 증가. batch 1건이 무겁고(R=25, 33관절×25프레임 JSON 파싱+대량 INSERT) 동시성 증가분이 대부분 **큐 대기(지연)로 흡수**됨. 이미 천장 부근이거나 그 너머에서 포화. → 백엔드 batch 적재 경로가 "안 터지는 tier"라는 §1 전제는 **부분적으로만 성립**: 안 죽지만(대부분 OK) 동시성 100에선 느려지고 일부 끊김.

⚠️ 미해결 / 주의:
- **이 ramp 수치는 천장 측정으로 미흡** — step 집계라 "어느 동시성에서 평탄해지나"를 못 봄. **고정 동시성별(c=10,20,...,100) 측정** 또는 per-step(`ramp.json` rawData 의 timestamp 구간 분할) 분석으로 천장·에러 시작점 특정 필요.
...
#### 공통 관찰
- **에러 100건은 모든 측정에서 동일** — 코드·JVM 상태와 무관하게 동시성 100 구간 `Unavailable` 100건 고정 재현. §7.5 의 커넥션 리셋은 batch 개선과 **독립적인 별도 문제**(미해결). batch 는 throughput·지연을 개선했으나 이 에러는 못 잡음 → 커넥션 풀/keepalive 별도 추적 필요.
...
| SLO 수치 | 🟡 ② 실측 + batch 개선 완료 (§7.5·7.6) — ramp p99 4,198ms·RPS 48.4·에러 0.98%. **JdbcTemplate batch 로 throughput +99%·p50 −64%·p99 −37%**(공정측정). SLO "콜백 p99<20ms" 는 작업량 정의 불일치로 **재정의 필요**. 에러 100건 원인·천장 특정·R 실측 미완 | 합격 기준 |
