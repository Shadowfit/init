...
| 2 | 로컬 캐시 | ✅ **구현+실측 완료**(FeedbackTemplateService) | — (완료) | 없음(§2-2-2 참고, 후속 후보는 별개) |
...
**현재 상태**: `FeedbackTemplateService.getTemplatesByExercise`에 Caffeine + Spring `@Cacheable`(cache-aside) 적용 완료. 아래는 착수 전 분석(§2-2 원문 유지, 히스토리 보존) → §2-2-2에 실측 결과.
...
**남은 것**: 테스트용 유저(`cachebench@test.com`) 정리 여부, 커밋 여부, `exercises`(임계값) 캐싱 후속 착수 여부 — 전부 미결정, confirm 대기.
...
> 0번은 이 문서 본론(8개 성능 시그널)과 카테고리가 다름 — **성능 최적화가 아니라 기능 갭 해소**. `reports`가 §2-2-1에서 확인했듯 쓰기 경로 자체가 없어 실제 세션 완료 시 404가 나는 상태라, 다른 항목보다 먼저 짚어야 할 전제로 올려둠.

0. **BE-03 착수(GPT/Claude 리포트 자동 생성)** — [`22-backend-tasks-detail.md`](../tasks/22-backend-tasks-detail.md) 기준 `GptFeedbackService` 신설 + LLM 호출 후 `Report` 저장 + 세션 종료 콜백 연결(추정 6h). **선결 미결정**: OpenAI vs Anthropic 선택(같은 문서 "리스크/의존"에 `decisions/llm-provider.md` 분리 추천됨). 이게 돼야 `reports`가 캐싱이든 뭐든 논의 가능한 상태가 됨.
1. ~~**로컬 캐시(Caffeine)** — `FeedbackTemplateService` 대상, 작고 명확한 스코프, 히트율 실측 가능.~~ **✅ 완료(2026-07-11)** — §2-2-2 실측 결과 참고. `exercises`(임계값) 등 후속 캐싱 후보는 별개 미결정.
2. **Resilience4j 서킷브레이커** — 이미 OPEN이던 항목 재확인, 비동기 골격 위에 얹기만 하면 됨.
3. **connection 설정 문서화** — `maximum-pool-size=10` 명시(코드 변경 아닌 설정 동기화).
4. (선택) **tcpdump** — §7.7 재현 한정.
5. (선택) **GC 반증 카드** — 튜닝 아닌 "GC 아니었다"는 증거 보강.
6. **k6 / netty 전환** — 비추천, 필요 시 언급만.
