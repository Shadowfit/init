...
| 2 | 로컬 캐시 | ✅ **구현+실측 완료**(FeedbackTemplateService, exercises) | — (완료) | 없음(§2-2-2·§2-2-3 참고) |
...
1. ~~**로컬 캐시(Caffeine)** — `FeedbackTemplateService` 대상, 작고 명확한 스코프, 히트율 실측 가능.~~ **✅ 완료(2026-07-11)** — §2-2-2 실측 결과 참고. ~~`exercises`(임계값) 등 후속 캐싱 후보는 별개 미결정.~~ **✅ exercises도 착수+완료(2026-07-11)** — §2-2-3 참고(evict 필요한 케이스로 차별화, AI 서버 미연동 갭은 별개로 정직하게 남김).
