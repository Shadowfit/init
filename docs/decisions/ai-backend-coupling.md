...
상태: **분기 9개 중 6개가 이미 닫혔다 — «OPEN» 은 낡은 표기** (상태줄 재정리 2026-08-17)
      아래 표가 실제다. 근거는 §11 결정 로그 + 다른 문서로 넘어간 분기들.

| 분기 | 지금 상태 | 어디서 닫혔나 |
|---|---|---|
| **A** 콜백 신뢰성 | 🟢 **사실상 이동** — A6(운영 알람)만 배포 시점 보류로 남고, 신뢰성 축은 **outbox 로 구현·실측까지 끝났다** | [`outbox-reliable-messaging.md`](./outbox-reliable-messaging.md) (2026-07-29, PR #60·#63·#67) |
| **B** proto 중복 | ✅ **닫힘 — B1 유지 + CI 드리프트 체크** (B3 는 빌드 컨텍스트 충돌로 폐기) | §11 (2026-07-19), `.github/workflows/proto-sync-check.yml` |
| **C** `exercise_type` 일반화 | 🔶 **의도적 보류** — 스쿼트 단일 기간 동안 열지 않는다 | [[project_squat_first]] |
| **D** AI 세션 상태 저장 위치 | ⚠️ **전제가 바뀌었다** — D1(in-memory 유지)이 디폴트였으나 세션 재개·AI 상태가 그 뒤 확정·구현됐다. **재검토 대상** | [`session-resume-and-ai-state.md`](./session-resume-and-ai-state.md) (2026-07-31 확정·구현) |
| **E** 통신 프로토콜 | ✅ **닫힘 — E1 gRPC 유지** | §11 · 이후 TTS batch 도 gRPC 로 통일(2026-05-26) |
| **F** 자세 데이터 영속화 | ✅ **닫힘 — F1, 이미 구현·동작 중** | §11 (2026-05-23) |
| **G** `GetFinalPoseData` | ✅ **닫힘 — G1 폐기**(proto 에서 제거) | §11 (2026-05-23) |
| **H** 카메라 프레임 송신 경로 | ✅ **닫힘 — H2(프론트 → AI 직결) 채택** | §11 (2026-05-24) |
| **I** 인증 토큰 흐름 | 🟡 **분리됨** — I1 잠정 채택 후 논의가 전용 문서로 넘어갔다(ㄱ 값 분리 **적용 완료 2026-08-09**, **I2 전환은 여전히 미결정**) | [`ai-auth-token-flow.md`](./ai-auth-token-flow.md) |

      🔴 **여기서 지금 열려 있는 건 사실상 D(재검토)와 I2(전환 시점) 둘뿐이다.** §9 «종합 권장» 표와 §10 «미결 질문» 7개는
      **2026-05-23 시점 원형**이라 A·B·F 항목이 이미 낡았다 — 본문을 읽을 때 이 표를 먼저 볼 것.
작성: 2026-05-23 · 상태줄 재정리: 2026-08-17
...
> 🔀 **2026-08-09 — 이 분기는 [`ai-auth-token-flow.md`](./ai-auth-token-flow.md) 로 이관됐다.**
> 아래 표·추천은 **2026-05-24 시점의 기록**이고, 그 뒤 전제를 바꾸는 사실 4가지가 드러났다([#134](https://github.com/Shadowfit/init/issues/134)):
> ① `EXPO_PUBLIC_` 토큰은 «탈취» 가 아니라 앱 번들에 **배포**된다 ② 같은 값이 채널 ②③(서버간 gRPC)의 인증이라 횡단된다 ③ 내부 RPC 4개에 **세션 소유권 검증이 없다** ④ JWT 가 **HS256 대칭**이라 I3 는 «검증 키» 가 아니라 **서명 키**를 넘기는 일이다.
> **현행 비교·추천은 이관 문서를 볼 것.** 아래는 이력으로 남긴다.
...
**미결 질문**:
- AI 컨테이너 재시작 빈도? 무중단 배포 요구사항이 있는가? — 🔴 **여전히 미측정, 그리고 배포 전엔 원천적으로 측정 불가**(2026-08-26, §11). 부하 rig로 잴 수 있는 값이 아니라 실제 운영 장애 이력이 있어야 나오는 값인데 배포 대상이 0대다
- ~~동시 활성 세션 수가 단일 인스턴스 한도(메모리/CPU) 안에 들어오는가?~~ → ✅ **답 나옴 (2026-08-26)**. [`ai-load-budget.md §7`](./ai-load-budget.md) — 물리 8코어 단독 156세션·동거 89~105세션(⚠️ **보정값 없음**(08-26 이전 라운드) — 라운드 간 **±10% 폭** 안에서만 읽을 것, [인용 규칙 ㉠ §8](./round-to-round-nonreproducibility.md#8--채택--인용-규칙--2026-09-14-사용자-결정)). 이 문서 작성 당시 걱정하던 규모(N=5)를 20배 넘게 초과 달성해서, **동시성은 D2 격상 사유가 아니다**
...
- **2026-08-26 (분기 D — D1 유지 재확인, 근거 정리)**: [`session-resume-and-ai-state.md §6-2`](./session-resume-and-ai-state.md)의
  "C(AI 상태 영속화, Redis 등)를 언제 볼지" 미결정 항목을 논의하다 여기로 왔다.
  - **미결 질문 둘 중 하나가 답이 났다** — 동시 활성 세션 수는 D2 격상 사유가 아니다(§7 갱신,
    `ai-load-budget.md §7`이 N=5 트리거를 20배 넘게 초과 달성했다고 이미 정정해뒀는데 이 문서만
    안 따라왔었다).
  - **남은 하나(AI 재시작 빈도)가 진짜 게이트인데, 배포 전엔 잴 방법이 없다** — 부하 rig가 만드는
    값이 아니라 실제 운영 장애 이력이 필요한 값이고, 배포 대상이 0대라 그 이력 자체가 없다.
  - **그래서 지금 D2(또는 C)를 만드는 것은 비용은 확정, 이득은 확률 미상인 채로 미리 지불하는
    선택이다.** 비용(Redis 컴포넌트·체크포인트 코드·운영 부담)은 지금 당장 들지만, 이득은 AI가
    세션 도중 실제로 죽어야만, 그것도 사용자가 그 세션을 재개하려 할 때만 실현된다. 장애가 안
    나면 이득은 0이고 비용만 남는다 — 확률을 모르는 상태에서 확정 비용을 거는 것은 손해 쪽에
    가깝다.
  - 게다가 A+B(§5, 2026-07-31 구현·검증 완료)가 이미 싸게 대부분의 가치(세션·rep 수 이어짐)를
    가져가고 있어서, C가 추가로 벌어주는 건 "재개 직후 폼 판정이 잠깐 부정확할 수 있다" 정도의
    작은 잔여분뿐이다. 남는 이득도 작은데 확률도 모른다.
  - **결론 — D1 유지, 재검토 조건은 그대로 "AI 재시작 빈도 실측"** 이고 이건 배포 이후로 미뤄진다.
    새 결정은 아니고(§9 디폴트가 이미 D1), 근거를 사용자와의 대화로 재확인·박제한 것.
- **2026-08-17 (상태줄 재정리, 새 결정 없음)**: 상태줄이 **«OPEN (사용자 결정 대기)»** 로 남아 있었는데,
  이 로그만 봐도 **B·E·F·G·H 는 닫혔고** A 는 [`outbox-reliable-messaging.md`](./outbox-reliable-messaging.md) 로,
  I 는 [`ai-auth-token-flow.md`](./ai-auth-token-flow.md) 로 **논의가 옮겨간 뒤였다.**
  즉 문서는 살아 있는데 **상태줄이 「전부 미결」이라고 잘못 말하고 있었다** — 「무엇이 남았나」를 물으면
  실제로 남은 D·I2 대신 분기 9개가 통째로 후보로 올라왔다. 그래서 상태줄에 분기별 현황 표를 넣었다.
  ⚠️ 재정리 중 하나가 걸렸다 — **분기 D(AI 세션 상태 저장 위치)의 디폴트 D1(in-memory 유지)은
  세션 재개 기능이 없다는 전제 위에 서 있었는데, 그 기능이 2026-07-31 에 확정·구현됐다.**
  D 는 «보류» 가 아니라 **재검토 대상**이다. §9·§10 은 2026-05-23 시점 원형으로 둔다.
