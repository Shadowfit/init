...
- §4 가정 상한(≤ 2,200행)에서 **한 자릿수 ms**. 요청당 다른 두 쿼리(현재 streak 페이지 ≈ 0.4 ms(§10 재인용 — 단 이 재인용은 첫 페이지가 §10 모양이 아니었던 #761 을 놓쳤다. 2026-09-23 첫 페이지를 §10 모양으로 고침)·이번 주 range scan 0.07 ms — 같은 판에서 확인)보다 이 쿼리가 지배적이지만, 슬로우 로그 부류가 아니다.
