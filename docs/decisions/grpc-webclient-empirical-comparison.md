...
| "이진화라 빠르다"(지도교수 조언) | 🔴 **여전히 못 받친다(4차)** — 4차에서 AI 프로세스가 REST 경로에서 호출당 1~3 cpu-ms 를 더 쓰는 것이 나왔지만, 그 REST 미러는 **JSON→pydantic→proto 조립→같은 gRPC 서비서**를 부르는 «A 를 감싼 겹» 이라(`internal_analysis.py:87-106`) 형식 비교가 아니다. 114KB 의 대부분이 문자열 필드 하나라 이진화가 이득 볼 자리도 없다. «인코딩» 자체는 Spring 쪽(판별 불가)·AI 쪽 어디서도 안 재졌다 — 결과 README §4-2 |
