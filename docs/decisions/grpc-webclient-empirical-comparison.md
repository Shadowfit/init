...
| "이진화라 빠르다"(지도교수 조언) | 🔴 **여전히 못 받친다(4차→5차 정정)** — 4차는 AI 쪽 +1~3 cpu-ms 를 «미러가 gRPC 를 감싼 겹의 대가» 로 읽었는데, **5차가 반증했다**([결과](../../loadtest/results/ai-call-native-rest-aws-2026-09-14/README.md) §2): 겹을 뺀 네이티브 팔이 미러와 겹친다. 그 1~3 cpu-ms 는 **REST 경로 자체**의 값이다 — 다만 «형식(protobuf↔JSON 인코딩)» 의 값인지 «FastAPI 경로(표준 JSON 파서·pydantic·스레드풀)» 의 값인지는 5차도 안 갈랐다. 114KB 의 대부분이 문자열 필드 하나라 이진화가 이득 볼 자리는 여전히 없고, 오히려 그 문자열을 중첩 JSON 으로 풀어 보내면 **더 비쌌다**(C−D). 즉 «gRPC 가 빠르다» 는 여기서 «FastAPI 의 REST 입구가 gRPC 입구보다 호출당 1.5~2.5 cpu-ms 비싸다» 까지만 말할 수 있다 |

즉 **지연을 이유로 gRPC 를 유지한다는 근거는 세 라운드를 거치며 얇아졌다.** 반대로 §2 의
「라우팅 이원화」 논거는 3차에서 홉이 싸다는 것이 확인돼 **강해졌다.**

> 🟡 **5차(2026-09-14) 뒤의 정직한 문장**: «지연을 이유로» 는 얇아진 채 그대로다(재부착 한 곳 +2~4ms). 그러나 **«REST 로 가면
> AI 쪽 호출당 1.5~2.5 cpu-ms · 포화 처리량 −13~16% 를 낸다» 는 미러의 인공물이 아니라 실제 대가**로 확정됐고, 네이티브로
> 다시 짜도 안 줄었다. 줄이려면 파서·검증 경로를 바꿔야 하는데 그건 잰 적 없다. 이 대가와 라우팅 이원화 해소를 저울에 올리는 것이 채택 판단이다.
...
nginx 디스크 버퍼링은 설정 한 줄로 0건. 남은 측정 후보는 «REST 고유 비용의 내부 분해»(파서·검증 경로)뿐이고 그건 채택 뒤
최적화 축이지 채택 입력이 아니다. **결정은 사용자 몫 — 이제 표는 다 찼다.**
