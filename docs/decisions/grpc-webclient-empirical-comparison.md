...
1차 실측(§10)이 끝났지만 **채택은 아직 미결이다.** 이유는 §10.2 다 — 결론을 가르는 자리
(큰 페이로드의 프로토콜 비용)가 «판정 불가» 로 나왔고, 그 답은 프로덕션 클라이언트로 다시
재야 나온다(§10.4). 지금 확정된 것은 «nginx 홉이 싸다» 와 «작은 요청 차이는 1ms 안팎» 둘뿐이고,
그 둘만으로 7개 RPC 의 거취를 정하지는 않는다.
