...
| **개별 세션 삭제** (미구현, §1-1) | ❌ 없음 — 향후 기능 | **세션 1건 × ~750행** — 소규모 고정 | ✅ 안전 — 동기 `DELETE`로 충분, B1 방식이어도 무방 (법적 요건도 즉시 동기 삭제가 가장 확실히 충족) |
...
- **2026-07-20**: §5-1 반영 — 개별 세션 삭제(향후 기능)는 소규모(세션당 ~750행)라 동기 삭제로 충분, 별도 배치 불필요. 회원 탈퇴(대량)만 B5 적용 대상.
