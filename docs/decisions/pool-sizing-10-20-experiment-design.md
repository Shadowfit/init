...
### 10-5. §9-3·§10-4 미검증 목록 실행 결과 (2026-09-04)

3대를 실제로 띄워 실행했다. 확인된 것:

- systemd `Environment=` sed 치환 → `assert_pool()`이 재기동 직후 매번 기대값을 그대로
  읽어 **실전에서 반영 확인됨**(16판 전부)
- health 대기 상한(60초) — 원격 DB 조건에서도 매 재기동이 여유 있게 통과, 상한을 못 채운 적 없음
- App↔DB 분리가 끝까지 도는 것 — 이번에 처음 확인됨(16판 전부 `ok=15000/fail=0`)

새로 드러난 것(실행 전엔 못 본 것):
- **세션 시드 누락** — DB·App을 분리하며 `p6-target`이 하던 세션 901~1900 시드 단계를
  옮길 곳을 놓쳤다. 1차 실행이 16판 100% `SESSION_NOT_FOUND`로 죽은 뒤 발견 → `bootstrap.sh`
  `ROLE=db`에 `SEED_MULTI_SESSIONS=1` 옵션으로 수정(커밋 `1173eb72`)
- `backend/gradlew` git 실행 비트 누락(100644) → `bash gradlew` 우회 + 비트 수정(`a1f0ac67`)
- 이 AMI가 `root@` SSH를 기본 차단 — 저장소의 `TARGET_SSH`/`DB_SSH` 관례(root@ 직결)와
  안 맞아 박스의 `authorized_keys`를 직접 정리(스크립트 변경 아님, 박스 설정)

## 11. 결과 요약 (2026-09-04) — [전체 결과](../../loadtest/results/pool-sizing-10-20-aws-2026-09-04/README.md)

- **RPS**: 10~20 전 구간 거의 평평(반복 분포 겹침 판정 기준) — pool=10만 나머지 넷과 안 겹침
- **HikariCP acquire 대기**(더 선명한 신호): 10→20 사이 **단조 감소**(판당 평균 9.48ms → 0.43ms,
  약 20배). 겹침 판정으로는 10<12<15(≈17)<20 — **15와 17은 서로 안 갈린다**
- **가용성 게이트**: `timeout_total` 위반 0, 전 판 무결
- **Q3 답**: 10과 15 사이엔 실효 차이가 있다(15가 근거 없는 관성값은 아니다) — 다만 15가
  20보다 못한 것도 같은 기준으로 확인돼 「15가 최적」의 근거는 아니다
- 🔴 **채택(15 유지·상향·하향)은 이 문서가 결정하지 않는다** — [[feedback_user_decides_not_claude]]
- ⚠️ 08-09 4점 baseline과 절대 비교 불가(아키텍처·인스턴스 패밀리 상이, §9-4) — 이 라운드는
  **자기 안에서의 상대 비교만** 유효
- 인스턴스 3대 terminate 확인 완료

이 문서는 여기서 닫는다 — 남은 것은 위 결과를 어떻게 쓸지(운영값 조정 여부)뿐이고, 그건
사용자 결정 사항이다.

## 12. §9(2대) 구성도 실제로 실행됐다 — 별도 세션, 같은 날 (2026-09-04)

> 📌 **두 결과(§11 3대 · §12 2대)의 비교 분석**: [`pool-sizing-10-20-topology-comparison.md`](./pool-sizing-10-20-topology-comparison.md) —
> 방향은 같다(pool=10이 나쁘고 20 근처가 좋다, acquire 대기시간 기준)지만 절대 크기·선명도가
> 10배 이상 다르다. 원인(동거·원격DB·인스턴스 패밀리)은 이 두 라운드만으로 분리 못 한다.

**§9-4가 "비교가능성 문제로 폐기"라고 기록한 §9(2대 p6-target+p6-loader) 구성이, 같은 날
다른 세션에서 실제로 EC2에 띄워져 돌았다.** 사용자가 그 세션에서 §9 그대로(4대 아님, 3대
아님, 원안 2대) 실행을 선택했다 — §9-4의 "폐기" 기록과 이 사용자 결정이 서로 다른 세션에서
동시에 진행됐다는 뜻이다. 두 실행 다 유효한 데이터이므로 어느 쪽이 "맞다"고 이 문서가 판정하지
않는다 — 아키텍처가 다른 두 조건에서 각각 답이 나온 것으로 둔다.

**결과**: [`../../loadtest/results/pool-sizing-10-20-manual-2026-09-04/README.md`](../../loadtest/results/pool-sizing-10-20-manual-2026-09-04/README.md)

- **pool=10과 현재 운영값 pool=15가 RPS·p99 둘 다 구분되지 않는다.** 10~20 구간은 사실상
  평평하다(10쌍 중 9쌍이 반복 분포 겹침, 유일한 예외 12↔17은 간격이 pool=15 자체의 판간
  변동보다도 작다)
- 가용성 게이트(`timeout_total` 증가) 위반 **0건**
- §9-4가 지적한 비교가능성 캐비어트(DB·App 동거, AI 유무, 인스턴스 타입 c7i vs t4g)는
  그대로 유효하다 — 절대 RPS는 08-09 baseline과 비교하지 않았고, 이 실행 안의 pool 간
  상대 비교(Q1·Q3)만 결론으로 썼다
- 실행 코드는 동시 세션 git 인덱스 경합으로 저장소 커밋을 거치지 않고 로더 박스에 직접
  올린 스크래치 사본이었으나, 이후 diff로 대조해 커밋 `a303daf4`의
  `loadtest/measure_poolsizing_10_20.sh`와 실행 로직이 100% 동일함을 확인했다

**§10·§11(3대 구성) 결과와 비교할 때 유의**: §10은 App을 bare jar+systemd로 DB와 분리한
조건, 이 §12는 p6-target 한 박스(MySQL+Spring+AI 동거) 조건이다. 두 결과가 다르게 나오면
그 차이 자체가 "DB·App 분리 여부가 풀 사이징 결론에 영향을 주는가"라는 정보가 된다 —
아직 그 비교는 이 문서에서 수행하지 않았다.
