...
#### 4-3-0-1. 세 번째 용처 — 세션 완료 자동 글 (2026-09-14)

`OutboxEventType.SESSION_COMPLETED` 이 붙었다(`social-cheer-and-group-feed.md` §4-4). 앞의 둘과 다른 점은 **상대가 바깥이 아니라 같은 DB 의 다른 애그리거트(그룹)** 라는 것이다 — 같은 트랜잭션에 INSERT 하면 아웃박스가 필요 없어 보이지만, 그러면 세션 완료 트랜잭션이 그룹 N행의 `FOR UPDATE` 락과 seq 채번·멤버십 규칙을 떠안는다. 애그리거트 경계를 이벤트로 넘기는 쪽을 택했고, «1초 늦어도 안 깨지는 결과» 라 at-least-once 로 충분하다.

이 용처가 처음 드러낸 것 하나: **수신자 멱등성이 없는 상대**다. AI 는 `already_active`·`not_found` 로, Expo 는 «두 번 울려도 된다» 로 재배달을 흡수했는데, `group_events` 는 같은 글이 두 번 생기면 사용자에게 그대로 보인다. 그래서 수신 쪽에 키를 만들었다 — `group_events.source_id` + UNIQUE `(group_id, event_type, source_id)`(V19). 발행기는 exists 로 거르고 UNIQUE 는 동시 재발행의 최종 방어선이다. **아웃박스에 새 상대를 붙일 때 «재배달을 누가 흡수하나» 는 매번 답해야 하는 질문**이고, 상대가 못 하면 이렇게 상대 쪽 스키마에 키를 두는 게 답이다.

또 하나: 한 행이 그룹 여러 개를 한 트랜잭션에 잠근다(발행 단위 = 행 전체, 부분 커밋 없음). 발행기 안에서 그룹을 둘 이상 잠그는 첫 경로라 **group_id 오름차순** 규약을 뒀다 — 다른 경로는 1개만 잠그므로 오름차순이면 순환이 없다. `SessionCompletedFeedFanoutRaceTest`(race 프로파일)가 이를 실 MySQL 에서 확인한다.
...
| **송신이 예외를 던짐** 🆕 (#759) | → `PENDING`(재시도) / 상한 초과 시 `FAILED` | RETRY 결과와 같은 길 — `retry_count` 를 올린다. 전엔 «크래시» 행처럼 회수에 맡겼는데 회수는 `retry_count` 를 안 올려 **영구 예외(독 메시지)가 §3-1 의 «내장 FAILED 처리» 를 비켜 갔다.** 결과 기록 단계(DB) 예외는 여전히 «크래시» 행대로 |
...
## 결정 로그
- 2026-09-23(#759, 사용자 confirm — 이슈 후보 1): §4-3-1 표에 «송신이 예외를 던짐» 행 추가. `dispatch()` 예외를 RETRY 로 세어 재시도 상한·`onGivenUp` 안으로 넣는다. 결과 기록 단계 예외는 회수 대기 그대로(DB 에 못 쓰는 상황이라 셀 수도 없다). 후보 2(회수 시 카운트)는 크래시 회수와 예외를 못 가르고, 후보 3(지표만)은 무한 반복을 그대로 둬서 제외.
